...
/**
 * DataScout Pro - Batch 1 Premium Prospects LIVE
 * Envoi RÉEL cold emails ultra-personnalisés
 * Date: 2026-04-26 14:00
 * REVENUE EXECUTION SLOT
 */
...
if (!resendKey || resendKey === 'your_resend_api_key_here') {
  console.error('❌ RESEND_API_KEY manquant ou invalide');
  process.exit(1);
}

console.log('🚀 DataScout Pro - Batch 1 Premium **LIVE EXECUTION**');
console.log('📅 Date:', new Date().toISOString());
console.log('🎯 Prospects:', prospects.length);
console.log('⚡ Mode: REAL SEND\n');

// Fonction d'envoi email via Resend API
...
// Envoi séquentiel avec délai entre chaque email
async function sendBatch() {
  ...
  for (let i = 0; i < prospects.length; i++) {
    ...
    try {
      console.log(`\n📧 Email ${i + 1}/${prospects.length} - ${prospect.company}`);
      console.log(`   To: ${prospect.email}`);
      console.log(`   Subject: ${prospect.subject}`);
      ...
      // Délai 2s entre chaque email (rate limiting)
      ...
      console.log(`   ❌ FAILED - ${error.status || 'Network error'}`);
...
(async () => {
  ...
  const logFile = path.join(__dirname, 'prospection', `batch-1-premium-results-${Date.now()}.json`);
  fs.writeFileSync(logFile, JSON.stringify(results, null, 2));
  console.log(`\n💾 Results saved: ${logFile}`);
  
  console.log('\n\n🎯 NEXT ACTIONS:');
  console.log('1. Monitor Resend dashboard for opens/clicks');
  console.log('2. Check inbox for replies');
  console.log('3. Prepare follow-up emails (J+3)');
  console.log('4. Update revenue-tracking.json');
})();
